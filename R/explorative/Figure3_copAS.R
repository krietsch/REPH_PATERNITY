...
di = merge(di, d[, .(obs_id, ID_year, ID2sex = sex, ID2copAS = copAS, ID2copAS_sep = cop)], 
           by.x = c('obs_id', 'ID2'), by.y = c('obs_id', 'ID_year'), all.x = TRUE)

di = merge(di, d[, .(obs_id, ID_year, ID1sex = sex, ID1copAS = copAS, ID1copAS_sep = cop, author, year_, datetime_, 
...
ds1 = ds[ID2 == ID1_1st_partner & ID1copAS == 1 & ID1sex == 'M', 
         .(ID1, ID2, diff_obs_initiation = diff_obs_1st_initiation, type = '1st', datetime_, ID1copAS_sep)]
ds2 = ds[ID2 == ID1_2nd_partner & ID1copAS == 1 & ID1sex == 'M', 
         .(ID1, ID2, diff_obs_initiation = diff_obs_2nd_initiation, type = '2nd', datetime_, ID1copAS_sep)]
dss = rbind(ds1, ds2)
...
# ignore who initiated 
dss[ID1copAS_sep %like% 'A', cop_ := 'A']
dss[ID1copAS_sep %like% 'S', cop_ := 'S']


ggplot(data = dss) +
  geom_bar(aes(diff_obs_initiation, fill = cop_), width = width_, color = bar_line, size = bar_line_thickness)
